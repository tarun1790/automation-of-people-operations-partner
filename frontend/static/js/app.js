...
async function loadRiskRadar() {
    try {
        ...
        data.department_risk_matrix.forEach(dept => {
            const badgeClass = dept.risk_level === 'CRITICAL' ? 'badge-red' :
                               dept.risk_level === 'HIGH' ? 'badge-amber' :
                               dept.risk_level === 'MODERATE' ? 'badge-blue' : 'badge-green';
            ...
            card.innerHTML = `
                <div>
                    <div class="flex items-center justify-between mb-2">
                        <span class="font-bold text-sm text-slate-900">${dept.department}</span>
                        <span class="badge ${badgeClass}">${dept.risk_level} RISK</span>
                    </div>
                    <div class="text-xs text-slate-500 mb-2">Staff: <strong>${dept.headcount}</strong> | Risk Index: <strong>${dept.risk_score}/100</strong></div>
                    <div class="space-y-1 mb-3">
                        ${dept.detected_signals.map(s => `<div class="text-xs text-slate-700 flex items-start"><span class="text-amber-600 mr-1.5">•</span><span>${s}</span></div>`).join('')}
                    </div>
                </div>
                <div class="pt-2 border-t border-slate-100 text-xs text-slate-600 flex justify-between items-center">
                    <span>Overtime: <strong>${dept.metrics.avg_overtime_hrs}h/mo</strong></span>
                    <button onclick="openSimulationForDept('${dept.department}')" class="text-blue-700 hover:underline font-medium">Test Scenario →</button>
                </div>
            `;
...
async function loadAIInsights() {
    try {
        ...
        insights.forEach(ins => {
            const card = document.createElement('div');
            card.className = 'card p-4 border-l-4 border-l-blue-700 flex flex-col justify-between';
            card.innerHTML = `
                <div>
                    <div class="flex justify-between items-start mb-1">
                        <span class="text-xs uppercase font-bold text-blue-800">${ins.badge}</span>
                        <span class="text-[11px] bg-slate-100 text-slate-600 px-2 py-0.5 rounded font-medium border border-slate-200">Confidence: ${ins.confidence}%</span>
                    </div>
                    <h4 class="text-sm font-semibold text-slate-900 mb-1">${ins.target}</h4>
                    <p class="text-xs text-slate-600 mb-2.5">${ins.finding}</p>
                    <div class="bg-slate-50 p-2.5 rounded border border-slate-200 mb-3 space-y-1">
                        <div class="text-[11px] font-semibold text-slate-500 uppercase">Supporting Data:</div>
                        ${ins.evidence.map(e => `<div class="text-xs text-slate-700 flex items-start"><span class="text-blue-600 mr-1.5">•</span><span>${e}</span></div>`).join('')}
                    </div>
                    <div class="text-xs text-slate-800 mb-3"><strong>Recommended Step:</strong> ${ins.recommended_action}</div>
                </div>
                <div class="flex gap-2 pt-2 border-t border-slate-100">
                    <button onclick="switchTab('actions')" class="btn-primary flex-1">Review in Action Center</button>
                    <button onclick="switchTab('simulation')" class="btn-secondary">Test Scenario</button>
                </div>
            `;
            container.appendChild(card);
        });
